...
Examples:
    {"type":"nature","stat":"attack","amount":1}
...
$functions = array(
    ...
    'robot_function_on-core-nearby' => function($objects){
        ...
        $find_robots_of_interest = function($player)
            use(&$robots_of_interest, &$noticed_robot_strings, $this_battle, $this_robot, $boost_type){
            ...
            foreach ($robots AS $key => $robot){
                ...
                if (!empty($robot->robot_core) && $robot->robot_core == $boost_type
                    || $boost_type === 'none' && empty($robot->robot_core)){
...
$functions['rpg-robot_check-skills_battle-start'] = function($objects) use ($functions){
    return $functions['robot_function_on-core-nearby']($objects, true);
};
