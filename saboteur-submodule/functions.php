...
$functions = array(
    ...
    'rpg-robot_check-skills_update-saboteurs' => function($objects){
        ...
        // Turn ON the heal-blocking feature of this skill
        // by adding this robot's ID to the player's anti_recovery list
        // and turn it OFF by removing it from the list
        $anti_recovery_added = false;
        $anti_recovery_robots = $this_player->get_value('anti_recovery_robots');
        if (empty($anti_recovery_robots)){ $anti_recovery_robots = array(); }
        if ($skill_is_active && !in_array($this_robot->robot_id, $anti_recovery_robots)){
            $anti_recovery_robots[] = $this_robot->robot_id;
            $anti_recovery_added = true;
        } elseif (!$skill_is_active && in_array($this_robot->robot_id, $anti_recovery_robots)){
            $anti_recovery_robots = array_diff($anti_recovery_robots, array($this_robot->robot_id));
        }
    ...
    'rpg-skill_disable-skill_before' => function($objects){
        ...
        $anti_recovery_removed = false;
        $anti_recovery_robots = $this_player->get_value('anti_recovery_robots');
        if (empty($anti_recovery_robots)){ $anti_recovery_robots = array(); }
        if (in_array($this_robot->robot_id, $anti_recovery_robots)){
            $anti_recovery_robots = array_diff($anti_recovery_robots, array($this_robot->robot_id));
            $anti_recovery_removed = true;
        }
        $this_player->set_value('anti_recovery_robots', $anti_recovery_robots);
...
$functions['rpg-robot_check-skills_battle-start'] = function($objects) use ($functions){
    return $functions['rpg-robot_check-skills_update-saboteurs']($objects);
};
