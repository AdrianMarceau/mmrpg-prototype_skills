    ...
    'rpg-robot_check-skills_end-of-turn' => function($objects){
        ...
        if (!empty($this_player_robot)){
            foreach ($this_player_robot AS $robot_key => $robot_info){
                ...
                $holding_anti_stat_booster = $robot->robot_item === $anti_stat_token.'-booster' ? true : false;
                $anti_stat_item = $robot->has_item() && $holding_anti_stat_booster ? true : false;
                if ($anti_stat_value > 0 || $anti_stat_value < 0 || $anti_stat_item){
                    $positions_of_interest[] = array(
                ...
                } elseif ($holding_anti_stat_booster) {
                    $robot->set_counter('item_disabled', 3);
                }
                unset($robot);
            }
        }
        ...
        $certain_text = count($positions_of_interest) === 1 ? 'a power anomaly' : 'power anomalies';
        ...
        $this_battle->events_create($this_robot, false, $this_robot->robot_name.'\'s '.$this_skill->skill_name,
            $this_robot->print_name().' took notice of '.$certain_text.' on the battlefield...',
            array(
        ...
        foreach ($positions_of_interest AS $key => $stat_to_remove){
            ...
            // Only reset stats if there are stats to reset (otherwise this is about an item)
            $already_kicked_in = false;
            if (!empty($stat_to_remove['value'])){

                // If this robot has a stat-based skill, display the trigger text separately
                $already_kicked_in = true;
            ...
            // Regardless, if the target was holding a relevant booster module, disable it until end of turn
            if ($stat_to_remove['robot']->has_item()
                && $stat_to_remove['robot']->robot_item === $stat_to_remove['stat'].'-booster'){

                // Disable the item for the rest of the turn and a few more
                $stat_to_remove['robot']->set_counter('item_disabled', 3);
                ...
                if (!$already_kicked_in){
                    $already_kicked_in = true;
                    $trigger_text = $this_robot->print_name().'\'s '.$this_skill->print_name().' skill kicked in! ';
                    $trigger_text .= '<br />'.$stat_to_remove['robot']->print_name().'\'s item was preemptively disabled!';
                } else {
                    $trigger_text = 'The skill prevents future disturbances too... ';
                    $trigger_text .= '<br />'.$stat_to_remove['robot']->print_name().'\'s item was disabled!';
                    $trigger_options['canvas_show_this_skill'] = false;
                }
                $this_battle->events_create($this_robot, false, $this_robot->robot_name.'\'s '.$this_skill->skill_name, $trigger_text, $trigger_options);
                $this_robot->reset_frame();
