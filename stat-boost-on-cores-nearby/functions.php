...
Required:
    - cores (string: cutter/impact/freeze/etc.)
...
Examples:
    ...
    {"cores":"nature,water","stat":"attack","amount":2}
        would boost attack by two stages when a nature or water core is on the field
...
$functions = array(
    ...
    'robot_function_on-cores-nearby' => function($objects){
        ...
        $find_robots_of_interest = function($player)
            use(&$robots_of_interest, &$noticed_robot_strings, $this_battle, $this_robot, $boost_cores){
            ...
            foreach ($robots AS $key => $robot){
                ...
                $core = !empty($robot->robot_core) ? $robot->robot_core : 'none';
                $core2 = !empty($core) && !empty($robot->robot_core2) ? $robot->robot_core2 : '';
                if (in_array($core, $boost_cores) || in_array($core2, $boost_cores)){
    ...
    'skill_function_onload' => function($objects){
        ...
        // Validate the "cores" parameter has been set to a valid value
        $allowed_core_types = array_keys(rpg_type::get_index(false, false, false, false));
        $allowed_core_types = array_merge($allowed_core_types, array('damage', 'recovery', 'experience'));
        if (!isset($this_skill->skill_parameters['cores'])
            || !in_array($this_skill->skill_parameters['cores'], $allowed_core_types)){
            error_log('skill parameter "cores" was not set or was invalid ('.$this_skill->skill_token.':'.__LINE__.')');
            if (isset($this_skill->skill_parameters['cores'])){
                error_log('cores = '.print_r($this_skill->skill_parameters['cores'], true));
            }
